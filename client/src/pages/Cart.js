  ...
  return (
    <div className="cart-page-kfc">
      <div className="cart-header-kfc">
        <a href="/Desserts" className="back-menu-btn">Quay lại menu</a>
          ...
          {cartItems.length === 0 ? (
            ...
            cartItems.map((item) => (
              <div key={item.id} className="cart-item-kfc">
                <img src={`http://localhost:8000/images/${item.image}`} alt={item.name} className="cart-item-image-kfc" />
                <div className="cart-item-info-kfc">
                  <div className="cart-item-row1-kfc">
                    <span className="cart-item-oldprice-kfc">{item.oldPrice ? `${item.oldPrice.toLocaleString()}đ` : ''}</span>
                    <span className="cart-item-price-kfc">{item.price.toLocaleString()}đ</span>
                  </div>
                  ...
                  <div className="cart-item-actions-kfc">
                    <a href={
                      item.category && item.category.toLowerCase() === "dest"
                        ? `/Desserts/${item.id}`
                        : item.category && item.category.toLowerCase() === "drink"
                        ? `/Drinks/${item.id}`
                        : `/Foods/${item.id}`
                    } className="cart-item-action-kfc">Xem chi tiết</a>
                    <button className="cart-item-action-kfc">Chỉnh sửa</button>
                    <button className="cart-item-action-kfc remove-button" onClick={() => handleRemove(item.id)}>Xóa</button>
                  </div>
                  <div className="cart-item-qty-kfc">
                    <button className="qty-btn-kfc" onClick={() => handleDecrease(item.id)}>-</button>
                    <span>{item.quantity}</span>
                    <button className="qty-btn-kfc" onClick={() => handleIncrease(item.id)}>+</button>
                  </div>
                </div>
                <div className="cart-item-total-kfc">{(item.price * item.quantity).toLocaleString()}đ</div>
              </div>
            ))
          )}
        </div>
